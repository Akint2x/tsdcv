...
public:
	...
	int getIndexOf(T item) noexcept
	{
		for (auto i = 0; i < length; i++)
			if (items[i] == item)
				return i;
	...
	void remove(int index) noexcept
	{
		if (index >= length)
			return;
		int sizeObj = sizeof(items[index]);
		void* add1 = (void*)((int)items + (index * sizeObj));
		void* add2 = (void*)((int)items + ((index + 1) * sizeObj));
		int size = (length - index - 1) * sizeObj;
		void* add3 = (void*)((int)add2 + size);
		memmove(add1, add2, size);
		length--;
	}
